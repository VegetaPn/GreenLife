...
function init(){
   ...
   document.getElementById("iToAddress").onclick = choseAddress;
}
...
function validate(){
	...
	if(consigne == ""||regione == ""||address == ""||zip == ""){
		alert("请填满信息！");
		return false;
	}
...
function change(v) {
    ...
    for(var i = 0; i<s.length; i++){
    	var region = document.getElementById("iRegione");
    	if(document.getElementById(s[i]).selectedIndex>0){
    		if(i == 0){
    			region.value ="";
    		}
    		region.value += document.getElementById(s[i]).value+" ";
    	}
    }
}

function getAddress(){
    document.getElementById("iRegione").value = document.getElementById("s1").value+" "+
...
function setup() {
    ...
    change(0);
    //document.getElementById(s[s.length -1]).onchange = getAddress;
}
